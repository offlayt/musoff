...
from message import NOT_IN_DB, PROCESSING, DOWNLOADING, UPLOADING, ALREADY_IN_DB, LYRICS_NOT_FOUND
from dbsessions import session, SongRequest
from handlers.album import Album
from handlers.song import Song
from utils import CLIENT, DB_CHANNEL_ID, BOT_ID
...
async def track_lyrics_callback_query(event: events.CallbackQuery.Event):
    ...
    if lyrics is None:
        await event.respond(LYRICS_NOT_FOUND)
    else:
