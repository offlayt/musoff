...
from message import DOWNLOADING, UPLOADING, PROCESSING, ALREADY_IN_DB, NOT_IN_DB, SONG_NOT_FOUND
from dbsessions import session, User, SongRequest
from handlers import SPOTIFY, GENIUS
from utils import DB_CHANNEL_ID, CLIENT, BOT_ID
...
class Song:
    ...
    async def song_telethon_template(self):
        message = f'''
🎧 Название :`{self.track_name}`
🎤 Артист : `{self.artist_name}{self.features()}`
...
[ID]({self.uri})     
        ...
        buttons = [[Button.inline(f'📩 Скачать трек', data=f"download_song:{self.id}")],
                   [Button.inline(f'🖼️ Скачать обложку', data=f"download_song_image:{self.id}")],
                   [Button.inline(f'👀 Альбом', data=f"album:{self.album_id}")],
                   [Button.inline(f'🧑‍🎨 Авторы альбома', data=f"track_artist:{self.id}")],
                   [Button.inline(f'📃 Текст песни', data=f"track_lyrics:{self.id}")],
